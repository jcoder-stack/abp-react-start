...
/** Configure the generated API client once at startup with PROCESS-WIDE, request-invariant values only (the BFF baseUrl, or a server-side fetch that itself reads per-request cookies/tenant/culture). This is a module-level singleton — never store per-request or per-tenant state here; keep that inside fetchFn. */
